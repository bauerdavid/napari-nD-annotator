...
class QObjectListWidgetItem(QListWidgetItem):
    ...
    def on_double_click(self, viewer, colormap):
        if self.parent.crop_image_layer is not None:
            ...
            viewer.window.remove_dock_widget(self.parent.widget)

        self.parent.crop_image_layer = Image(self.image_layer.data[self.bbox_idx], name="Cropped Image", colormap=colormap)
        self.parent.crop_image_layer.translate = tuple(self.bounding_box[0, :])
        viewer.add_layer(self.parent.crop_image_layer)
        self.parent.crop_mask_layer = Labels(self.mask_layer.data[self.bbox_idx], name="Cropped Mask")
        ...
        self.parent.widget = SliceDisplayWidget(viewer, self.parent.crop_image_layer, self.parent.crop_mask_layer, self.channels_dim)
        qt_widget = viewer.window.add_dock_widget(self.parent.widget)
        qt_widget.setFloating(True)
        @self.parent.crop_mask_layer.mouse_drag_callbacks.append
    ...
    def on_data_change(self, event):
        ...
        self.parent.widget.update_slider_ranges()
...
class ListWidget(QListWidget):
    def __init__(self, viewer, name_template, bounding_box_layer, image_layer, mask_layer, channels_dim):
        ...
        if name_template.find("#") >= 0:
            ...
            names = [name_template % i for i in range(len(icons))]
        else:
        ...
        self.mask_layer = mask_layer
        self.itemDoubleClicked.connect(lambda x: x.on_double_click(viewer, image_layer.colormap))
    ...
    def bounding_box_change(self, layer, event):
        ...
        if np.shape(previous_data) != np.shape(self.bounding_box_layer.data):
            ...
            if self.widget is not None:
                for p in self.widget.projections:
                    p.update_overlay()
