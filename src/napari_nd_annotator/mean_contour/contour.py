...
def delta_d(contours, q_mean, rsqrts):
    energy = 0
    denominator = 0
    ...
    for i in range(len(contours)):
        ...
        for j in range(targetRepr.shape[0]):
            targetRepr[j,:] -= q_mean[j,:]
            energy += targetRepr[j,0]*targetRepr[j,0]+targetRepr[j,1]*targetRepr[j,1]
            targetRepr[j,:] *= targetSrv[j]
            numerator += targetRepr[j,:]
            numeratori += targetRepr[j,:]
            denominatori += targetSrv[j]*targetSrv[j]
            targetSrv[j] -= rsqrts[j]
            denominator += targetSrv[j]*targetSrv[j]

        targetCont.centroid[0,0] = numeratori[0,0]
        targetCont.centroid[0,1] = numeratori[0,1]
        targetCont.centroid /= denominatori
